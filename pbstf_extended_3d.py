...
lst_positions = ti.field(dtype=tm.vec3, shape=Nmax)
neg_grad = ti.field(dtype=tm.vec3, shape=Nmax)
...
@ti.kernel
def restore_lst_positions():
	for i in range(N[None]):
		lst_positions[i] = positions[i]

@ti.kernel
def get_positions():
	for i in range(N[None]):
		positions[i] = lst_positions[i] + delta_positions[i]

@ti.kernel
def get_neg_energy_grad(b: ti.template()) -> tm.vec3:
	'''
	Calculates -(grad C)^T C - (grad C_surf)^T 1 evaluated on positions and stores in b.
	'''
	distance_sos.fill(0)
	b.fill(0)
	# density constraint
	...
	for i in range(N[None]):
		...
		for j in range(n_neighbors[i]):
			...
			surface_sum[i] += surface_weight * triangle_area(t0, t1, t2)
...
@ti.kernel
def scaled_accumulate(a: ti.template(), scale_a: float, b: ti.template(), scale_b: float):
	'''
...
def proximal_solve():
	constraint_packed = get_neg_energy_grad(neg_grad)
	alpha = mass[None] / dt ** 2
	delta_positions_len = .5 * alpha * get_squared_length(delta_positions)
	scaled_accumulate(delta_positions, beta / (alpha + beta), neg_grad, 1. / (alpha + beta))
	return constraint_packed[0], constraint_packed[1], constraint_packed[2], delta_positions_len

@ti.kernel
def get_squared_length(a: ti.template()) -> float:
	res = 0.
	for i in range(N[None]):
		res += tm.length(a[i]) ** 2
	return res
...
if __name__ == '__main__':
	...
	if cmd_args.frame > 1:
		export_obj(vis_p, local_mesh[:tri_cnt, :], os.path.join(dir_name, 'particles_0.obj'), particles_only=False)
	...
	for frame in range(cmd_args.frame):
		advance()
		init_neighbor_searcher()
		restore_lst_positions()	# lst_positions = positions
		...
		for iter in range(max_iter):
			...
			get_surface_normal()
			get_local_meshes()
			density_constraint, distance_constriant, surface_constraint, inertia = proximal_solve()	# update delta_positions
			ti.sync()
			acc_time += time.time() - st_time
			constraint_sos[iter] = density_constraint + distance_constriant + surface_constraint + inertia
			dist2ball[iter] = distance_to_perfect_ball()
			if iter % 100 == 0:
				print(f'Iteration {iter}: {constraint_sos[iter]} = {density_constraint} + {distance_constriant} + {surface_constraint} + {inertia}, dist2ball: {dist2ball[iter]}, time: {acc_time}')
				tot_time += acc_time
				if cmd_args.frame == 1:
					tri_cnt = get_visualization_data(vis_p, local_mesh)
					export_obj(vis_p, local_mesh[:tri_cnt, :], os.path.join(dir_name, f'particles_iteration_{iter}.obj'), particles_only=False)
				acc_time = 0.
			st_time = time.time()
			get_positions()	# positions = lst_positions + delta_positions
			init_neighbor_searcher()
			ti.sync()
			acc_time += time.time() - st_time
		st_time = time.time()
		...
		ti.sync()
		acc_time += time.time() - st_time
		density_constraint, distance_constriant, surface_constraint, inertia = proximal_solve()
		constraint_sos[max_iter] = density_constraint + distance_constriant + surface_constraint + inertia
		dist2ball[max_iter] = distance_to_perfect_ball()
		print(f'Iteration {max_iter}: {constraint_sos[max_iter]} = {density_constraint} + {distance_constriant} + {surface_constraint} + {inertia}, dist2ball: {dist2ball[max_iter]}, time: {acc_time}')
		tot_time += acc_time
		
		tri_cnt = get_visualization_data(vis_p, local_mesh)
		print(tri_cnt)
		export_obj(vis_p, local_mesh[:tri_cnt, :], os.path.join(dir_name, f'particles_{frame + 1}.obj' if cmd_args.frame > 1 else f'particles_iteration_{max_iter}.obj'), particles_only=False)
